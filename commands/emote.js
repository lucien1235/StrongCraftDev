...
module.exports.run = async (client, message, args) => {
    message.delete();
    if (!args[0]) return message.channel.send('Please send an emoji.').then(msg => {msg.delete(3000)});;
    try {
        ...
        if (error.message === 'TypeError: Cannot read property \'1\' of null') {
            message.reply('Give me an actual emote.').then(msg => {msg.delete(3000)});;
        }
    }
}
